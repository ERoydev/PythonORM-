...
from django.db.models import Value, Avg, Sum, Count, CharField, Q, F
...
def product_quantity_ordered():
    ...
    ).values('name', 'total').order_by('-total').exclute(total=None)
...
def ordered_products_per_customer():
    orders = Order.objects.prefetch_related('products', 'customer', 'products__category')
    result = []
...
def give_discount():
    ...
    products_to_be_discounted = Product.objects.filter(query).order_by('-price', 'name')
    products_to_be_discounted.update(
        price=F('price') * 0.7
    )
...
products = Product.objects.raw('SELECT * FROM main_app_product')
